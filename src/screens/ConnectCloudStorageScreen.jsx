...
import { Image, Keyboard, Linking, Pressable, ScrollView, Text, TextInput, View } from 'react-native';
...
export default function ConnectCloudStorageScreen({ initialCloudName = '', initialUploadPreset = '', saving, onSave, onBack }) {
  ...
  return <ScrollView className="flex-1 bg-canvas px-6 pt-8" contentContainerClassName="pb-12">
    ...
    <Pressable disabled={saving} onPress={save} className="mt-8 h-14 items-center justify-center rounded-2xl bg-brand"><Text className="font-extrabold text-white">{saving ? 'Checking connection…' : 'Save and continue'}</Text></Pressable>
  </ScrollView>;
}
