...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!req.user.cloudName) return res.status(400).json({ message: 'Connect cloud storage before registering media' });
    if (String(b.cloudName || '').trim() !== req.user.cloudName) return res.status(403).json({ message: 'Media must use your linked cloud storage account' });
...
router.get('/my-uploads', async (req, res) => { const cloudName = req.user.cloudName; if (!cloudName) return res.json([]); const items = await Media.find({ userId: req.user._id, cloudName }).sort({ createdAt: -1 }).lean(); res.json(items.map(media => ({ ...media, imageUrl: signedViewUrl(media).url, mediaUrl: signedViewUrl(media).url }))); });
